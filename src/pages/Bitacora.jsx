...
// Utilidad mínima para dar formato tipo Markdown básico (negritas, itálicas, código y saltos de párrafo)
function renderFormatted (raw) {
  if (!raw) return null
  // Limpia indentación inicial y recorta
  const trimmed = raw.replace(/^[\s\n]+|[\s\n]+$/g, '')
  // Separa párrafos por líneas vacías (dos o más saltos)
  ...
  return paragraphs.map((p, idx) => {
    let html = p
      ...
      // Itálicas *texto*
      .replace(/(^|[\s([])\*(?!\*)([^*]+?)\*(?=\s|$|[.,;:!?)[\]])/g, '$1<em>$2</em>') // itálicas mejoradas
      // Código en línea `code`
      .replace(/`([^`]+?)`/g, '<code>$1</code>')
      // cursivas con _texto_
...
const bitacoraEntries = [
  {
    ...
    content: `En esta primera semana realizamos una reunión de planificación para analizar la consigna del TP2 y definir los objetivos del trabajo.
...
Finalmente, planificamos la estructura de carpetas y la arquitectura general de la aplicación, priorizando componentes reutilizables, separación lógica por vistas (*pages*) y un sistema de navegación con **React Router DOM**.`,
  },
  {
    ...
    content: `Comenzamos la implementación del proyecto creando los primeros componentes base: **Sidebar**, **Home**, **Bitácora** y **TeamCard**.
...
Se comenzó a experimentar con **Framer Motion** y **Animate.css** para aplicar animaciones sutiles en los títulos y tarjetas.`,
  },
  {
    ...
    content: `En esta semana implementamos la lectura de datos desde **JSON locales** y también la conexión a una **API pública externa**.
...
Esta parte del trabajo nos permitió practicar el uso de *hooks* como **useEffect**, **useState** y el manejo de *fetch API*.`,
  },
  {
    ...
    content: `Se avanzó en la implementación de la sección **Juegos NES**, conectando un archivo local **nesgames.json** con un listado de 20 títulos clásicos.
...
En cuanto al diseño, se pulieron las transiciones, se ajustaron las animaciones de scroll y se mejoró la compatibilidad responsiva con pantallas pequeñas.`,
  },
  {
    ...
    content: `Esta semana se incorporó una de las mejoras más importantes del proyecto: el **emulador Nostalgist.js**.
...
Esta etapa implicó pruebas con distintos ROMs (.nes) y ajustes en la carga de archivos desde la carpeta */public/roms/*.`,
  },
  {
    ...
    content: `En la última semana nos enfocamos en la **documentación completa del proyecto** y en los últimos retoques visuales y funcionales.
